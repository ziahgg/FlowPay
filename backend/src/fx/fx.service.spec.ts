...
describe('FxService', () => {
  ...
  describe('convert', () => {
    it('delegates to TradeExecutionService with a user[from] source and FX_CONVERT entry type', async () => {
      ratesService.getRate.mockResolvedValue({
    ...
    it('propagates a ledger rejection (e.g. insufficient funds) unchanged', async () => {
      ...
      tradeExecutionService.executeSwap.mockRejectedValue(error);
